...
strippedpath = StripProjectDir(path.to_s)
ImagePath.setBundlePath( strippedpath +"/aut.sikuli")
ImagePath.setBundlePath( strippedpath +"/repository.sikuli");
require strippedpath + "/aut.sikuli/aut.rb"
require strippedpath + "/repository.sikuli/repository.rb"
   ...
   #strip all " and ()
    ...
    if sArray[1].include? ";"
       ...
       #regular function, with actions on objects
        ...
        send(sArray[0], sArray[1].chomp(';'))
    elsif sArray[1].include? ","            
...
module ConfigRead   
    ...
    def self.ProcessConfigLine(line)
       begin
            configline = line.chomp.split(":=")
            case  configline[0].to_s
                when 'LOGPATH'                   
